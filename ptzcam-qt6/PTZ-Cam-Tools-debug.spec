...
qt_dlls = [(b[0], os.path.getsize(b[1])) for b in a.binaries if b[0].startswith("Qt6")]
qt_dlls.sort(key=lambda x: -x[1])
for name, size in qt_dlls: print(f"  {name}: {size/1024/1024:.0f}MB")
import os
a.binaries = [
